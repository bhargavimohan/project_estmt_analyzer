...
def extract_desired_costs(pdf_text):
    try:
        ...
        print(f"Error extracting desired costs: {e}")
        return []
...
def classify_costs(desired_cost_list):
    try:
        ...
        deposits = []
        classified_costs_dict = {
            main_category.main_category: [] for main_category in main_categories
        }
        for sub_category_str in desired_cost_list:
            ...
            if "ALTER SALDO" in sub_category_str:  # record the old balance
                ...
                main_category_name = get_main_category_name(main_category_id)
        ...
        print(f"Error classifying costs: {e}")
        return {}
...
def compute_category_costs(classified_cost_dict):
    try:
        ...
        print(f"Error computing category costs: {e}")
        return {}
...
def verify_category_total_costs(final_cost_dict):
    try:
        ...
        if LHS == RHS:
            success_message = (
                "TADAAAA!!Individual costs combined match the New Balance 😊"
            )
            final_cost_dict.update({"E-stmnt Analyzer Status": success_message})
        else:
            warning_message = "Uh Oh!!We've encountered an issue 😬 Individual costs put together does not align with the New Balance in your e-statement"
            final_cost_dict.update({"E-stmnt Analyzer Status": warning_message})
        return final_cost_dict
    except Exception as e:
        print(f"Error verifying category total costs: {e}")
        return final_cost_dict


def process_pdf(estmnt_file_path):
    try:
        ...
        if is_pdf_reviewed is False:
            ...
            save_db_results(estmnt_file_path, output_json)
    except Exception as e:
